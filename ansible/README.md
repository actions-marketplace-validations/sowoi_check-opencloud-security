# Ansible playbooks for check-opencloud-security
...
> (`check_opencloud_security`), so running both would create conflicting
> configuration. Each role writes a marker file
> (`/etc/icinga2/conf.d/.check_opencloud_security_{native,docker}`) and
...
- `roles/opencloud_check_native/` - installs the plugin into a dedicated
  virtualenv via pip and symlinks it into the Nagios plugin directory.
- `roles/opencloud_check_docker/` - builds the Docker image on the target
...
$EDITOR inventory.ini   # set your Icinga2 hosts and opencloud_check_host
...
|:---------------------------------------|:----------------------------------------------------------|:--------------------------------|
| `opencloud_check_host`                 | OpenCloud server to scan (hostname or URL)                | `{{ inventory_hostname }}`      |
| `opencloud_check_proxy`                | HTTP/HTTPS proxy (optional)                                | `""` (unset)                    |
...
| `opencloud_check_retries`              | Retry attempts for transient network errors                | `2`                             |
| `opencloud_check_backoff_factor`       | Exponential backoff factor (seconds) between retries       | `0.5`                           |
| `opencloud_check_timeout`              | HTTP timeout in seconds per request                        | `10`                            |
| `opencloud_check_warning`              | Rating (0-5) at or below which the check warns             | `3`                             |
| `opencloud_check_critical`             | Rating (0-5) at or below which the check is critical       | `1`                             |
| `opencloud_check_check_hardening`      | Also report missing hardening measures and headers         | `false`                         |
...
| `opencloud_check_icinga_host_name`     | Existing Icinga2 `Host` object to attach the Service to     | `{{ inventory_hostname }}`      |
| `opencloud_check_icinga_conf_dir`      | Where to write the `.conf` files                           | `/etc/icinga2/conf.d`           |
| `opencloud_check_validate_config`      | Run `icinga2 daemon -C` after templating                   | `true`                          |
...
| Variable                        | Description                                       | Default                                |
|:---------------------------------|:----------------------------------------------------|:-----------------------------------------|
| `opencloud_check_repo_url`        | Git URL installed with pip                          | `https://github.com/sowoi/check-opencloud-security.git` |
| `opencloud_check_repo_version`    | Git ref (branch/tag/commit) to install              | `main`                                   |
| `opencloud_check_venv_path`       | Virtualenv location                                 | `/opt/check-opencloud-security/venv`     |
| `opencloud_check_plugin_dir`      | Nagios/Icinga2 plugin directory (symlink target)    | `/usr/lib/nagios/plugins`                |

### Docker role only

| Variable                          | Description                                        | Default                                  |
|:-----------------------------------|:------------------------------------------------------|:--------------------------------------------|
| `opencloud_check_repo_url`          | Git URL cloned to build the image from                | `https://github.com/sowoi/check-opencloud-security.git` |
| `opencloud_check_repo_version`      | Git ref (branch/tag/commit) to build                  | `main`                                     |
| `opencloud_check_clone_dest`        | Where the repo is cloned on the target host           | `/opt/check-opencloud-security/src`        |
| `opencloud_check_docker_image_name` | Docker image name                                     | `check-opencloud-security`                 |
| `opencloud_check_docker_image_tag`  | Docker image tag                                      | `latest`                                   |
| `opencloud_check_force_rebuild`     | Always rebuild the image, even without repo changes   | `false`                                     |
| `opencloud_check_icinga_user`       | Service account added to the `docker` group           | `icinga`                                    |
| `opencloud_check_docker_binary`     | Path to the Docker CLI used by the CheckCommand       | `/usr/bin/docker`                           |

## What each role does

### `opencloud_check_native`
...
3. Creates a virtualenv and installs `check-opencloud-security` into it
   with `pip install git+...`.
4. Symlinks the venv's `check-opencloud-security` executable into the
...
`opencloud_check_repo_version` to pick up a new release:
...
- Remove `{{ opencloud_check_icinga_conf_dir }}/check_opencloud_security*.conf`
  and the corresponding marker file, then reload icinga2.
- Native: remove `opencloud_check_venv_path` and the plugin-directory symlink.
- Docker: remove the built image (`docker rmi check-opencloud-security`) and
  `opencloud_check_clone_dest`, and optionally remove the icinga2 user from
  the `docker` group.
