...
| `check_pull_request.py` | Refuses a pull request without a `CHANGELOG.md` and `RELEASE.md` entry, or with a version change that is not forward, already tagged or misnamed in its commit subject. A local check before opening a pull request; no workflow runs it. |
