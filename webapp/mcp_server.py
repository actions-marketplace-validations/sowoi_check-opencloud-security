...
def build_mcp_server(app: Any, settings: WebSettings) -> MCPServer:
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "link-local address is refused and retrying will not help, and so "
            "is any address this deployment has been asked not to scan. "
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "waived, the findings the requester asked to ignore; and groups, "
            "the same open findings grouped by where the change is made - "
            "reverseProxy, identityProvider, opencloud, dnsZone - where each "
            "change lists the findings it resolves together, whether it "
            "resolvesSeveral, and the ratingAfter that change alone gives, "
            "with groupSummary naming the edits that close several at once. "
            "When the user is about to edit one system, answer from its "
            "group.\n\n"
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "are new; unchanged, the ones still open; newlyMeasured and "
            "noLongerMeasured, findings on one side only because the other "
            "scan did not make that check at all - not new, not fixed, and "
            "not a change to the instance; ratingChange; changes, "
            "the itemised list including any movement in the version and the "
            "support horizon; and both scans' ratings, versions and scan "
            "times. Both scans must describe the same instance: two "
            "different instances answer 422 and are not compared.\n\n"
