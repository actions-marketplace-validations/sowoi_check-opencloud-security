...
**Behebung:** Lass den Webserver Anfragen an OpenCloud weiterleiten, statt das Installationsverzeichnis auszuliefern. Deaktiviere Verzeichnisindizes zusätzlich, etwa mit `autoindex off` in nginx oder `Options -Indexes` in Apache. Siehe [Reverse Proxys](../reverse-proxy.md).
...
**Behebung:** Beende die Dateiauslieferung und prüfe anschließend, dass die genannten Pfade `404` liefern. Behandle tatsächlich lesbare Geheimnisse als offengelegt: Tausche betroffene Schlüssel und Zugangsdaten aus und untersuche einen möglichen unbefugten Zugriff.
...
**Behebung:** Entferne die öffentliche Portzuordnung und beschränke den Zugriff auf Loopback oder das erforderliche interne Netz.
...
**Behebung:** Entferne die öffentliche Portfreigabe und binde das Backend an Loopback oder ein privates Container-Netz, das nur der Reverse Proxy erreichen muss.
...
**Behebung:** Beschränke `OC_CORS_ALLOW_ORIGINS` auf die tatsächlich benötigten Ursprünge. Setze `OC_CORS_ALLOW_CREDENTIALS=false`, wenn keiner dieser Abläufe Zugangsdaten benötigt. Dienstspezifische Variablen wie `GRAPH_CORS_ALLOW_ORIGINS` und `OCS_CORS_ALLOW_ORIGINS` überschreiben die gemeinsame Vorgabe.
...
**Behebung:** Deaktiviere `TRACE` am Proxy. Apache bietet `TraceEnable off`; nginx lehnt die Methode normalerweise mit `405` ab. Bei weiterleitenden Regeln in nginx, Traefik oder Caddy beschränke die erlaubten Methoden entsprechend.
...
Ist unter diesem Ursprung kein Backend nachweisbar, erscheinen beide Befunde nicht. Der Scanner folgt keinem aus dem Dokument übernommenen Hostnamen, da sonst die gescannte Instanz das nächste Verbindungsziel bestimmen könnte. Einen separat betriebenen Editor musst du mit dafür geeigneten Prüfungen untersuchen. Siehe [ADR 0036](../../adr/0036-a-companion-service-is-probed-only-where-the-scan-was-pointed.md).

**Behebung:** Sperre den Administrationspfad am Reverse Proxy und veröffentliche nur die für den Editor benötigten Pfade. Verwende HTTPS für die Editor-Adressen.
