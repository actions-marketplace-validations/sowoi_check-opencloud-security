# Ejemplos prácticos del escáner de seguridad de OpenCloud

Estos ejemplos cubren las formas habituales de ejecutar el escáner. Sustituya
`opencloud.example.com` por la dirección de su instancia y analice solo
sistemas que tenga permiso para probar.

Los ejemplos más largos y específicos de cada plataforma tienen su propia
página: [Kubernetes](../kubernetes.md), [canalizaciones de CI](../ci.md),
[Prometheus y Grafana](../prometheus.md),
[adaptadores de webhooks](../webhook-recipes.md) y
[conjuntos de instancias](../many-instances.md).

<!-- TOC -->
* [Ejemplos prácticos](#worked-examples)
  * [Lo básico](#the-basics)
  * [Ejemplos de canales de publicación](#release-track-examples)
  * [Aceptar hallazgos que no se van a corregir](#accepting-findings-you-are-not-going-to-fix)
  * [Ambas cosas juntas, en un archivo de configuración](#both-together-in-a-configuration-file)
  * [Instancias que no están en internet](#instances-that-are-not-on-the-public-internet)
  * [Umbrales y notificaciones](#thresholds-and-notifications)
  * [Definición de comando de Icinga2](#icinga2-command-definition)
  * [El escáner por sí solo](#the-scanner-on-its-own)
<!-- TOC -->
...
Recuerde que una versión *más reciente* no es automáticamente una versión *con
mejor soporte*: declarar `production` en una instancia que ejecuta una versión
rolling la notifica como *por delante* de su canal, no como actualizada en él,
y nunca como fin de vida, que se reserva para una versión por detrás de la
actual de su canal.
...
Esta es la forma preferible para todo lo permanente, porque una exclusión puede
llevar un comentario que explique por qué existe y cuándo debe revisarse:
...
Los mismos ajustes como variables de entorno, para un contenedor o una unidad
de systemd:
