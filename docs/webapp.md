...
| `COS_WEB_UPDATE_CHECK` | `true` | Ask PyPI whether a newer release of this service exists, for the operator's area only and at most every six hours. Set `false` with no outbound access |
| `COS_WEB_ADMIN_UPDATE_DIR` | *(unset)* | A directory shared with the `updater` service of `docker-compose.dockerhub.yml`. Set, the operator's area can install a newer release: it writes a request there and the updater pulls the image and recreates the web and worker containers, a short downtime. Unset, the area only says an update exists |
