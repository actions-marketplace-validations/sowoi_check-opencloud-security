"""
Read the OpenCloud release schedule from the published lifecycle page.

The admin documentation is the only place that states the release *type* at
all - the GitHub release list cannot tell a rolling release from a production
one - so this module turns that page into the same document that ships as
``data/release_schedule.json``.

It lives in the library rather than in ``scripts/`` because two very different
callers need exactly one parser. ``scripts/update_release_schedule.py`` runs
it in CI and commits the result; the web application runs it once a day and
keeps the result in Redis, so that a deployment does not tell visitors that
last month's release is the newest one. Neither may drift from the other, and
``scripts/`` is not part of the wheel.

Nothing here decides anything. It fetches, it parses, and it refuses to
return a document that does not look like the page it expects - the caller
...
LIFECYCLE_URL = LIFECYCLE_DOCUMENTATION_URL
...
# a caller is better off with the schedule it already has than with whatever
# fell out of a page that no longer looks like a release table.
MIN_LINES = 5
MAX_LINES = 500
MIN_MAJOR = 1
MAX_MAJOR = 200
    ...
    for label, rows in zip(parser.labels, parser.panels):
        ...
        for row in rows:
            ...
            if version is not None:
                ...
                releases.append((version, parse_release_date(row[1])))
...
def fetch_schedule_document(
    url: str = LIFECYCLE_URL, timeout: int = 30
) -> dict[str, Any]:
    """
    Read the lifecycle page and return the release schedule document.

    Raises :class:`ExtractionError` when the page cannot be fetched or does
    not look like the release tables it is supposed to be. A caller that gets
    an exception keeps the schedule it already had; there is no such thing
    here as a partial refresh.
    """
    try:
        ...
        raise ExtractionError(f"Could not read {url}: {exc}") from exc
    return build_document(extract(html), url)
